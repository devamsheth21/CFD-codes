 ...
 for (j = 1;j < NCELLJ;j++)
  {
    for (i = 1;i < NCELLI;i++)
    {
     PHI[j][i] = PHI_OLD[j][i] + (q_gen*DELT)/(rho*Cp)+ ((DELT / VOL[j][i])*( (aw[j][i]*PHI_OLD[j][i- 1]) + (as[j][i]*PHI_OLD[j-1][i]) + (ae[j][i]*PHI_OLD[j][i + 1]) + (an[j][i]*PHI_OLD[j+1][i]) -(ap[j][i]*PHI_OLD[j][i]) ));
    }
  }
}
void update()
...
void write()
{
	ofstream file;
	stringstream ss;
	ss << "RUN/TEMP_PROFILE_TIME_" << NSTEP*DELT << ".dat";
